...
class Environment
  ...
  def self.database
    unless @database
      ...
      @database.execute "CREATE TABLE IF NOT EXISTS skills(id INTEGER PRIMARY KEY AUTOINCREMENT, training_path_id INTEGER, name VARCHAR(29), description text, mastered BOOLEAN)"
    end
    @database
  end
end
